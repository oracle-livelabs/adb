...
<!---
This should only be used for select workshops where an AI model is provided 
--->
...
* Enable access to an OpenAI model. This access will be revoked after the workshop.
...
You can deploy your resources to an existing compartment or create a new one. To create a new compartment:
...
Use an OCI Cloud Stack to set up your workshop environment by creating an ADB instance, upload the data to the instance, set up connectivity to OpenAI and install the Select AI demo application that was built using APEX.
...
Let's view the newly created stack and job. 

1.  From the Console, open the **Navigation** menu and click **Developer Services**. Under **Resource Manager**, click **Stacks**.
...
2.  Click the stack name. The **Stack details** page is displayed.

    ![Click Jobs](./images/stack-details-page.png "")

3.  In the **Jobs** section, click the job name. The **Job details** page is displayed.

    ![Job details page](./images/job-details.png "")

4.  As mentioned in the previous section, the keys and values are displayed in the **Outputs** section. This includes the following fields that you will use later in the workshop:
...
5.  You can use the **Logs** section to view the created resources such as the **moviestream** user. 
