...
The workshop uses the 'Autonomous JSON Database' cloud service. But everything that is shown here also works in other Oracle cloud and on-premise databases, except for the database API for MongoDB which is currently only available for Autonomous Database. 
