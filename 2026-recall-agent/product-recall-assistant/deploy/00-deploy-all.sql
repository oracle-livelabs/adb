...
prompt ============================================================
prompt Product Recall Assistant - Complete Database Deployment
prompt Start from an ADMIN SQLcl session connected to the target ADB.
prompt This single file contains all database setup phases for Labs 1 through 8.
prompt The deployment resets only the named workshop identities and objects.
...
begin
    if user != 'ADMIN' then
        raise_application_error(
            -20100,
            'Start this deployment from an ADMIN SQLcl session.'
        );
    end if;
    ...
    if dbms_db_version.version < 23 then
        raise_application_error(
            -20101,
            'Oracle AI Database 23ai or later is required.'
        );
    end if;
end;
/
...
declare
    l_missing varchar2(4000) := null;
    l_count   number;

    function package_or_public_synonym_exists(p_name varchar2)
        return boolean
    is
        l_exists number;
    begin
...
begin
    for r in (
        ...
        if not package_or_public_synonym_exists(r.object_name) then
            l_missing := l_missing || case when l_missing is null then '' else ', ' end || r.object_name;
        end if;
    end loop;
    ...
    if l_count = 0 then
        l_missing := l_missing || case when l_missing is null then '' else ', ' end || 'MDSYS.SDO_GEOMETRY';
    end if;
    ...
    if l_count = 0 then
        l_missing := l_missing || case when l_missing is null then '' else ', ' end || 'DATA_PUMP_DIR';
    end if;

    if l_missing is not null then
        raise_application_error(
            -20102,
            'Required database features are missing: ' || l_missing
        );
    end if;
end;
/
...
prompt --- Preflight: download the public Oracle embedding model ---
...
exception
    when others then
        raise_application_error(
            -20103,
            'Public ONNX model download failed; no workshop reset was performed: ' || sqlerrm
        );
end;
/
...
begin
    ...
    if not l_exists or l_file_size is null or l_file_size = 0 then
        raise_application_error(
            -20104,
            'The public ONNX model was not written to DATA_PUMP_DIR.'
        );
    end if;

    dbms_output.put_line('Public ONNX model is available: ' || l_file_size || ' bytes.');
end;
/

prompt --- Preflight: stop if a workshop identity has an active session ---
...
begin
    ...
    if l_session_count > 0 then
        raise_application_error(
            -20105,
            'Workshop identities have active sessions. Disconnect them and rerun; no sessions were terminated.'
        );
    end if;
end;
/

prompt --- Preflight: inventory the existing workshop boundary ---

select 'SCHEMA USER' as object_class,
...
prompt --- Reset: remove only the existing workshop boundary ---

begin
    declare
        l_app_user_count number;
        l_owner_count    number;
    begin
        select count(*)
        into   l_app_user_count
        from   dba_users
...
drop data role if exists recall_store_101_data_role;
drop data role if exists recall_region_ne_data_role;
drop data role if exists recall_lead_data_role;
drop end user if exists store_101_user;
drop end user if exists region_ne_user;
drop end user if exists recall_lead_user;
drop user if exists recall_app_user cascade;
drop user if exists recall_owner cascade;

begin
    begin
        execute immediate 'drop role recall_api_role';
    exception
        when others then
            if sqlcode not in (-1919, -1921) then
                raise;
            end if;
    end;

    begin
        execute immediate 'drop role recall_end_user_login';
    exception
        when others then
            if sqlcode not in (-1919, -1921) then
                raise;
...
prompt --- Recreate ADMIN-owned identities and platform grants ---

create user recall_owner
    identified by "&&shared_password"
...
alter user recall_owner grant connect through spatial$proxy_user;
...
declare
    procedure create_end_user(p_name varchar2) is
        l_sql varchar2(4000);
    begin
        l_sql := 'create end user ' || p_name ||
                 ' identified by "&&shared_password" account unlock schema recall_owner';
        execute immediate l_sql;
    end;
begin
    create_end_user('STORE_101_USER');
    create_end_user('REGION_NE_USER');
...
prompt The shared deployment password is also used for the final ADMIN reconnection.
set define on
whenever sqlerror exit sql.sqlcode rollback
set serveroutput on
set feedback on
...
prompt ADMIN bootstrap complete.
set define on


prompt --- Owner phase: relational schema and seed data ---

connect recall_owner/"&&shared_password"
set define on
whenever sqlerror exit sql.sqlcode rollback
set define off
set serveroutput on
...
prompt --- Facilitator phase: load the public ONNX embedding model ---
...
begin
    ...
    dbms_cloud_ai.create_profile(
        ...
        status       => 'ENABLED',
        description  => 'LLM profile for the Product Recall Assistant workshop.'
    );
...
declare
    l_status             batches.recall_status%type;
    l_case_status        recall_investigations.case_status%type;
    l_workflow_state     varchar2(40);
...
begin
    ...
    select case_status,
           json_value(case_data, '$.workflowState')
    into   l_case_status,
           l_workflow_state
    from   recall_investigations
    where  batch_id = 'B-482';

    select count(*), coalesce(sum(units_sent), 0)
    ...
    if l_status != 'CLEAR'
       or l_case_status != 'REVIEW'
       or l_workflow_state != 'REPORTED'
       or l_affected_stores != 120
       or l_units_sent != 2400
       or l_exposed_customers != 600
       or l_component_batches != 25
       or l_supplier_sites != 25 then
        raise_application_error(-20106, 'Seeded B-482 verification failed.');
    end if;

    dbms_output.put_line(
        'PASS: B-482 is CLEAR / REVIEW with 120 stores, 2400 units, 600 customers, 25 component batches, and 25 supplier sites.'
    );
end;
/

prompt ============================================================
prompt Facilitator deployment finished.
prompt Session left connected as RECALL_OWNER.
prompt Run Lab 1, then complete Labs 2 through 8 in order.
prompt Each lab creates its own Spatial, graph, vector, agent, API, security,
prompt and application objects.
prompt ============================================================
